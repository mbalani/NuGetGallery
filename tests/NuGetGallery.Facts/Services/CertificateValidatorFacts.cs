...
namespace NuGetGallery.Services
{
    public class CertificateValidatorFacts
    {
        ...
        {
            var exception = Assert.Throws<ArgumentNullException>(() => new CertificateValidator().Validate(file: null));
        ...
        public void Validate_WhenFileExtensionIsInvalid_Throws(string fileName)
        {
            ...
            var exception = Assert.Throws<ArgumentException>(() => new CertificateValidator().Validate(file));

            Assert.Equal("file", exception.ParamName);
            Assert.StartsWith("The file extension must be .cer.", exception.Message);
        }
        ...
        public void Validate_WhenStreamIsNull_Throws()
        {
            ...
            Assert.Equal("file", exception.ParamName);
            Assert.StartsWith("The file stream is invalid.", exception.Message);
        }
        ...
        public void Validate_WhenStreamIsNotSeekable_Throws()
        {
            using (var stream = new NonSeekableStream())
            {
                ...
                Assert.Equal("file", exception.ParamName);
                Assert.StartsWith("The file stream must be seekable.", exception.Message);
            }
        }
        ...
        public void Validate_WhenContentLengthIsTooSmall_Throws(int contentLength)
        {
            ...
            Assert.Equal("file", exception.ParamName);
            Assert.StartsWith("The file length is invalid.", exception.Message);
        }
        ...
        public void Validate_WhenContentLengthIsTooLarge_Throws()
        {
            using (var stream = GetDerEncodedCertificateStream())
            {
                ...
                Assert.Equal("file", exception.ParamName);
                Assert.StartsWith($"The file exceeds the size limit of {MaximumSizeInBytes} bytes.", exception.Message);
            }
        }
        ...
        public void Validate_WhenStreamIsPfx_Throws()
        {
            using (var stream = GetPfxStream())
            {
                ...
                var exception = Assert.Throws<InvalidDataException>(() => new CertificateValidator().Validate(file));
