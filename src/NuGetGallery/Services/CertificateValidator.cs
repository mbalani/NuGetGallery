...
namespace NuGetGallery
{
    public sealed class CertificateValidator : ICertificateValidator
    {
        ...
        public void Validate(HttpPostedFileBase file)
        {
            ...
            if (stream == null)
            {
                throw new ArgumentException(Strings.ValidateCertificate_InvalidStream, nameof(file));
            }

            if (!stream.CanSeek)
            {
                throw new ArgumentException(Strings.ValidateCertificate_StreamMustBeSeekable, nameof(file));
            }

            if (file.ContentLength <= 0 || stream.Length <= 0)
            {
                throw new ArgumentException(Strings.ValidateCertificate_InvalidFileLength, nameof(file));
            }
            ...
            {
                throw new InvalidDataException(Strings.ValidateCertificate_InvalidEncoding);
            }
        }
